...
function buildSystemPrompt(agent: any, agentId: string): string {
  ...
  lines.push('')
  lines.push('### HIGHEST PRIORITY: Specific Person Lookup')
  lines.push(`1. "tell me about [NAME]", "erzähl mir über [NAME]", "who is [NAME]", "show me [NAME]", "info about [NAME]" → query_contact_by_name(searchName: "[NAME]")`)
  lines.push(`   ⚠️ CRITICAL: When user mentions a SPECIFIC PERSON'S NAME → USE THIS FUNCTION FIRST`)
  lines.push(`   Examples: "Jim Betterman", "Sarah Smith", "John Doe", "Mike at Acme Corp"`)
  lines.push('')
  lines.push('### SECOND PRIORITY: Product/ICP Fit Queries')
  lines.push(`2. "best fit", "best match", "match my product", "ideal customers", "ICP fit", "target persona", "right fit", "good fit" → query_contacts_by_agent_fit(agentId: "${agentId}", threshold: 0.4)`)
  lines.push(`   ⚠️ CRITICAL: When user mentions "product", "best", "match", "ideal", "target" → USE THIS FUNCTION`)
  lines.push('')
  lines.push('### Other Query Types')
  lines.push('3. "never contacted", "not contacted yet", "fresh prospects" → query_never_contacted()')
  lines.push('4. "engaged contacts", "high engagement", "opened emails", "clicked links" → query_contacts_by_engagement()')
  lines.push('5. "from Germany", "CFO", "in construction industry", specific role/company/keyword → query_contacts_basic()')
  lines.push('6. "recently contacted", "last 30 days", "recently engaged" → query_contacts_by_recency()')
  lines.push('7. "enriched contacts", "with LinkedIn", "complete data" → query_contacts_by_enrichment()')
  lines.push('8. "bounced", "unsubscribed", "active contacts" → query_contacts_by_status()')
  lines.push('')
  ...
  lines.push('')
  lines.push('### Specific Person Lookup (HIGHEST PRIORITY)')
  lines.push('✅ "Tell me about Jim Betterman" → query_contact_by_name(searchName: "Jim Betterman")')
  lines.push('✅ "Erzähl mir über Jim Betterman" → query_contact_by_name(searchName: "Jim Betterman")')
  lines.push('✅ "Who is Sarah Smith?" → query_contact_by_name(searchName: "Sarah Smith")')
  lines.push('✅ "Show me info about John Doe" → query_contact_by_name(searchName: "John Doe")')
  lines.push('✅ "What do you know about Mike Johnson?" → query_contact_by_name(searchName: "Mike Johnson")')
  lines.push('✅ "Find contact named Lisa" → query_contact_by_name(searchName: "Lisa")')
  lines.push('')
  lines.push('### Product Fit / ICP Queries (SECOND PRIORITY)')
